...
def create_presentation():
    ...
    # Professional Color Palette (High contrast, enterprise board aesthetic)
    ...
    stats = [
        ...
        ("83% (5/6)", "Surfaced Fit/Drape Doubt", "5/6 interviewees surfaced fit uncertainty; 3/6 (50%) cited it as #1 blocker.", GREEN),
        ("6 / 6 (100%)", "Outside-App Workarounds", "All 6 participants left Myntra or relied on past orders/friends to resolve fit.", NAVY_DARK),
    ]
    ...
    p = tf_f.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    p.text = "30-Day Wishlist Buyer Conversion = (Unique users purchasing ≥1 item they wishlisted in ≤30 days of adding it) ÷ (Unique cohort users with ≥1 eligible wishlist add)"
    p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = WHITE

    # Metric Decomposition Formula Bar
    ...
    p.alignment = PP_ALIGN.CENTER
    p.text = "METRIC TREE: 30D Conversion = Wishlist Revisit × Intent Persistence × Decision Confidence × Wishlist → Bag × Bag → Purchase"
    p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = NAVY_DARK

    # 5 Hypothesized Funnel Stage Cards (Width 2.8in each)
    funnel = [
    ...
    tf = tx.text_frame; tf.word_wrap = True
    p = tf.paragraphs[0]
    p.text = "EXPLICIT NON-GOALS & BOUNDARIES (*HYPOTHESIZED FUNNEL)"; p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = AMBER; p.space_after = Pt(8)
    p = tf.add_paragraph()
    ...
    proc_steps = [
        ...
        ("5. AUDIT", "300 record manual verification; 90% AI–human label agreement."),
    ]
    ...
    dist_cards = [
        ("51.3%", "HIGH INTENT / HIGH FRICTION\n51.3% fall into High Intent / High Friction — the primary target segment (n=770).", ROSE, WHITE),
    ...
    p = tf.add_paragraph(); p.text = "named cross-brand fit as their primary #1 blocker"; p.font.size = Pt(14); p.font.color.rgb = NAVY_DARK
    ...
    add_card(slide5, 0.6, 3.3, 14.8, 0.5, bg_color=WHITE, border_color=BORDER_COLOR)
    tx_j = slide5.shapes.add_textbox(Inches(0.8), Inches(3.35), Inches(14.4), Inches(0.4))
    tf_j = tx_j.text_frame; tf_j.word_wrap = True
    p = tf_j.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    p.text = "USER DECISION JOURNEY: Wishlist Add → 'Will this fit me?' → Reddit / YouTube / Past Orders → Still Uncertain → Purchase Delayed"
    p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = ROSE
    ...
    add_table(slide5, 0.6, 3.9, 8.8, 4.3, p_headers, p_data, [0.5, 1.1, 1.6, 1.8, 1.8, 2.0])

    add_card(slide5, 9.6, 3.9, 5.8, 4.3, bg_color=WHITE, border_color=BORDER_COLOR, left_accent_color=ROSE)
    tx_w = slide5.shapes.add_textbox(Inches(9.8), Inches(4.05), Inches(5.4), Inches(4.0))
    tf_w = tx_w.text_frame; tf_w.word_wrap = True
    p = tf_w.paragraphs[0]
    p.text = "OBSERVED OUTSIDE-APP WORKAROUND LOOP"; p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = NAVY_DARK; p.space_after = Pt(6)
    ...
    tf_n = tx.text_frame; tf_n.word_wrap = True
    p = tf_n.paragraphs[0]
    ...
    p = tf_n.add_paragraph(); p.space_before = Pt(8)
    ...
    p.text = "CAUSAL A/B TEST EXPERIMENT DESIGN"; p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = ROSE; p.space_after = Pt(4)
    p = tf_exp.add_paragraph()
    p.text = "• Population: High-intent apparel wishlisters with ≥1 eligible fit-sensitive SKU   • Control: Existing Wishlist   • Treatment: Wishlist + FitCheck\n• Randomization: User-level A/B split   • Evaluation Window: 30 days   • Decision Rule: Launch only if 30-Day Conversion rises without inflating returns."
    p.font.size = Pt(14); p.font.color.rgb = SLATE_TEXT
    ...
    r_data = [
        [("False Fit Confidence → High Returns", True, NAVY_DARK), ("HIGH", True, ROSE), "MED", "AI overconfident without garment specs", "Show 3-tier confidence bands ('High', 'Moderate', 'Uncertain'); abstain if data <80%; gate rollout on return rate."],
