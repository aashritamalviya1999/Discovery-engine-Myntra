...
def create_presentation():
    ...
    # Executive Summary Banner (Aligned with Subtitle)
    ...
    # 5 Funnel Stage Cards (Cleaned Up: Removed Unsupported +14%)
    funnel = [
        ...
        ("3. Wishlist → Bag", "Commit shortlisted SKU to Bag", "Primary Outcome\nGoal: Uplift vs control", True),
    ...
    # Right Column: Opportunity Ranking Table (Legend integrated into Subtitle!)
    ...
    p = tf.add_paragraph(); p.text = "cited cross-brand fit as their primary blocker"; p.font.size = Pt(14); p.font.color.rgb = NAVY_DARK
    ...
    # ==========================================
    # SLIDE 8: Deployed MVP Prototype (Same-Category & UI Visual Demonstrator)
    # ==========================================
    ...
    # Left Column: Visual UI Wireframe Cards (Same-Category Jeans Anchor -> Jeans Target)
    ...
    p.text = "PROTOTYPE UI FLOW (SAME-CATEGORY MATCHING)"; p.font.size = Pt(16); p.font.bold = True; p.font.color.rgb = NAVY_DARK; p.space_after = Pt(10)

    # UI Step 1 Box
    p = tf_ui.add_paragraph()
    run = p.add_run(); run.text = "1. SET FIT ANCHOR: "; run.font.bold = True; run.font.color.rgb = ROSE; run.font.size = Pt(14)
    run = p.add_run(); run.text = "User inputs known-good fit item: [Roadster Slim Fit Jeans - Size 30 M]. Saved as personal reference anchor."; run.font.color.rgb = SLATE_TEXT; run.font.size = Pt(14)
    p.space_after = Pt(8)

    # UI Step 2 Box
    p = tf_ui.add_paragraph()
    run = p.add_run(); run.text = "2. FITCHECK SIGNAL: "; run.font.bold = True; run.font.color.rgb = AMBER; run.font.size = Pt(14)
    run = p.add_run(); run.text = "User taps 'Run FitCheck' on saved wishlist item [Highlander Cargo Jeans - Size 30 M]. System returns a High-Confidence fit result based on prototype logic."; run.font.color.rgb = SLATE_TEXT; run.font.size = Pt(14)
    p.space_after = Pt(8)
    ...
    # ==========================================
    # SLIDE 9: Define Success & Clean Taxonomy
    # ==========================================
    ...
    for idx, (m_tag, m_val, bg_c, tag_c, val_c) in enumerate(top_metrics):
        c_left = 0.6 + idx * 5.02
        add_card(slide9, c_left, 1.8, 4.76, 1.4, bg_color=bg_c, border_color=GREEN if bg_c == GREEN_BG else None)
        tx_box = slide9.shapes.add_textbox(Inches(c_left + 0.2), Inches(1.85), Inches(4.36), Inches(1.2))
        tf = tx_box.text_frame; tf.word_wrap = True
        p = tf.paragraphs[0]
    ...
    add_card(slide9, 0.6, 3.3, 14.8, 1.1, bg_color=WHITE, border_color=ROSE, left_accent_color=ROSE)
    tx_exp = slide9.shapes.add_textbox(Inches(0.8), Inches(3.35), Inches(14.4), Inches(1.0))
    tf_exp = tx_exp.text_frame; tf_exp.word_wrap = True
    p = tf_exp.paragraphs[0]
    p.text = "CAUSAL A/B TEST EXPERIMENT DESIGN & SAMPLE RIGOR"; p.font.size = Pt(14); p.font.bold = True; p.font.color.rgb = ROSE; p.space_after = Pt(2)
    p = tf_exp.add_paragraph()
    p.text = "• Population: High-intent apparel wishlisters with ≥1 eligible fit-sensitive SKU   • Control: Existing Wishlist   • Treatment: Wishlist + FitCheck\n• Randomization: User-level A/B split   • Evaluation Window: 30 days   • Sample Size: Determined using power analysis from Myntra's baseline conversion and predefined minimum detectable effect (MDE).\n• Decision Rule: Launch only if 30-Day Conversion rises without inflating fit returns."
    p.font.size = Pt(14); p.font.color.rgb = SLATE_TEXT

    # Metrics Taxonomy Table (Compact padding, ZERO Footer Overflow!)
    ...
    add_table(slide9, 0.6, 4.5, 14.8, 3.6, m_headers, m_data, [1.8, 3.2, 5.4, 4.4])
    ...
    output_pptx = r"C:\Users\sanja\.gemini\antigravity\scratch\myntra_wishlist_conversion\NL_Myntra_Final.pptx"
